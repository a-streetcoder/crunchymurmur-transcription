...
const { transcribeWav } = require('./whisper-cli');
...
function cliExecutableName(platform = process.platform) {
...
function resolveWhisperRuntime({
  ...
  const configuredCli = String(env.CRUNCHYMURMUR_WHISPER_CLI_PATH || '').trim();
  const configuredServer = String(env.CRUNCHYMURMUR_WHISPER_SERVER_PATH || '').trim();
  const cliCandidates = [configuredCli];
  const serverCandidates = [configuredServer];
  for (const directory of String(env.PATH || '').split(pathModule.delimiter)) {
    if (!directory) continue;
    cliCandidates.push(pathModule.join(directory, cliExecutableName(platform)));
    serverCandidates.push(pathModule.join(directory, serverExecutableName(platform)));
  }
  if (platform === 'win32') {
    const installRoot = env.LOCALAPPDATA || env.USERPROFILE || '';
    cliCandidates.push(pathModule.join(env.ProgramFiles || 'C:\\Program Files', 'whisper.cpp', 'whisper-cli.exe'));
    cliCandidates.push(pathModule.join(installRoot, 'whisper.cpp', 'whisper-cli.exe'));
    serverCandidates.push(pathModule.join(env.ProgramFiles || 'C:\\Program Files', 'whisper.cpp', 'whisper-server.exe'));
    serverCandidates.push(pathModule.join(installRoot, 'whisper.cpp', 'whisper-server.exe'));
  } else {
  ...
  const cliPath = [...new Set(cliCandidates.filter(Boolean))]
    .find((candidate) => usableExecutable(candidate, platform)) || '';
  const serverPath = [...new Set(serverCandidates.filter(Boolean))]
    .find((candidate) => usableExecutable(candidate, platform)) || '';
  return { cliPath, serverPath, bundled: false };
}
